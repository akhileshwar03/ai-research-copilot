...
def _encode_sources(sources: list[dict] | None) -> str | None:
    if not sources:
        return None
...
class RealtimeSessionService:
    def __init__(
        self,
        session_repo: RealtimeSessionRepository,
        message_repo: RealtimeMessageRepository,
            ...
            "sessions": [
                {
                    ...
                    "messages": [
                        {"role": m.role, "content": m.content, "sources": _decode_sources(m.sources)}
    ...
    def create_session(self, email: str, title: str, messages: list[dict], pinned: bool = False) -> dict:
        ...
        encoded = [{"role": m["role"], "content": m["content"], "sources": _encode_sources(m.get("sources"))} for m in messages]
        self.message_repo.create_many(session_id=session.id, messages=encoded)
        db.commit()
        db.refresh(session)
        logger.info("realtime_session_created email=%s session_id=%s", email, session.id)
    ...
    def update_session(
        self, session_id: int, user_email: str, title: str, messages: list[dict], pinned: bool = False
    ) -> dict:
        ...
        self.message_repo.create_many(session_id=session.id, messages=encoded)
        db.commit()
        logger.info("realtime_session_updated email=%s session_id=%s pinned=%s", user_email, session.id, pinned)
    ...
    def delete_session(self, session_id: int, user_email: str) -> dict:
        ...
        db.commit()
        logger.info("realtime_session_deleted email=%s session_id=%s", user_email, session_id)
