...
import { useDisplayName } from "@/features/shared/hooks/use-display-name";
 ...
 * Mounted once in the app-wide top bar (see TopNav) so account access is
 * consistent everywhere without every product's sidebar re-wiring it.
 *
 * 2026-09-20: this used to also render a "sidebar" layout (full-width,
 * bottom-anchored, menu opening upward) for use inside a per-product
 * sidebar, from before account access was consolidated into the single
 * shared TopNav above. Nothing rendered it any more — grepped every caller
 * — so it was deleted rather than left to silently drift out of sync with
 * fixes made to the one layout that's actually live (branded avatar, real
 * display name, aria-label). Top-right is also just the right call on its
 * own merits: it's the placement every comparable product uses, so it's
 * where a user already expects to find "my account" without thinking.
 */
...
function UserFooter({
  ...
  const displayName = useDisplayName(email);
  ...
  return (
    <div ref={ref} className="relative shrink-0">
      {open && (
        <div className="absolute right-0 top-full z-30 mt-2 w-64 overflow-hidden rounded-2xl border border-[var(--border-medium)] bg-[var(--surface-2)] shadow-2xl shadow-black/60 ring-1 ring-black/20">
          <div className="border-b border-[var(--border-subtle)] px-4 py-3.5">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="truncate text-[13px] font-medium capitalize text-[var(--text-primary)]">{displayName}</p>
          ...
          <div className="border-t border-[var(--border-subtle)] py-1.5">
            <MenuItem icon={<SignOutIcon />} label="Sign Out" destructive onClick={() => { setOpen(false); logout(); }} />
          </div>
        </div>
      )}
      <button
        ...
        aria-expanded={open}
        className={[
          "hover-surface flex max-w-[180px] shrink-0 items-center gap-2 rounded-full border border-[var(--border-medium)] py-1 pl-1 pr-2.5 transition",
          open ? "bg-[var(--surface-2)]" : "bg-[var(--surface-1)]",
        ].join(" ")}
      >
        {/* Branded (not flat-gray) so it reads as a deliberate account control, not
            inert chrome — same accent used for the Querex mark on the left, so it's
            the one other colored element in the bar and the eye lands on it. */}
        <div
          className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[12px] font-bold uppercase ring-1 ring-[var(--border-medium)]"
          style={{ backgroundColor: "var(--marketing-accent-soft)", color: "var(--marketing-accent-text)" }}
        >
          {initial}
        </div>
        {/* Name/email label, not just a bare initial — a new user scanning the bar
            for "where's my account" has a word to read, not just a shape to guess
            at. Hidden below sm, where the bar is already tight; the avatar alone
            (plus the aria-label/title) still fully identifies the control there.
            Uses the same display name as the dropdown header above — set once in
            Profile settings, it now actually shows up somewhere. */}
        <span className="hidden min-w-0 truncate text-[12.5px] font-medium capitalize text-[var(--text-primary)] sm:block">
          {displayName}
        </span>
        <svg
          className={["h-3.5 w-3.5 shrink-0 text-zinc-500 transition-transform", open ? "rotate-180" : ""].join(" ")}
